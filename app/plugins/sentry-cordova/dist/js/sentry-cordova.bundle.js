cordova.define("sentry-cordova.Sentry", function(require, exports, module) { 'use strict';
...
var esm = /*#__PURE__*/Object.freeze({
    ...
    get TransactionSamplingMethod () { return TransactionSamplingMethod; }
});
...
var esm$1 = /*#__PURE__*/Object.freeze({
    ...
    usingPerformanceAPI: usingPerformanceAPI,
...
function eventFromPlainObject(exception, syntheticException, rejection) {
    var event = {
        exception: {
            values: [
                {
                    type: esm$1.isEvent(exception) ? exception.constructor.name : rejection ? 'UnhandledRejection' : 'Error',
                    value: "Non-Error " + (rejection ? 'promise rejection' : 'exception') + " captured with keys: " + esm$1.extractExceptionKeysForMessage(exception),
                },
            ],
        },
        extra: {
    ...
    if (syntheticException) {
        var stacktrace = tracekit.computeStackTrace(syntheticException);
        var frames_1 = prepareFramesForEvent(stacktrace.stack);
        event.stacktrace = {
            frames: frames_1,
        };
...
var eventbuilder = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
function eventFromException(options, exception, hint) {
    ...
    esm$1.addExceptionMechanism(event, {
        handled: true,
        type: 'generic',
    });
...
function eventFromUnknownInput(exception, syntheticException, options) {
    ...
    if (esm$1.isErrorEvent(exception) && exception.error) {
        ...
        // eslint-disable-next-line no-param-reassign
        exception = errorEvent.error;
        event = parsers.eventFromStacktrace(tracekit.computeStackTrace(exception));
        return event;
    }
    if (esm$1.isDOMError(exception) || esm$1.isDOMException(exception)) {
        ...
        var message = domException.message ? name_1 + ": " + domException.message : name_1;
        event = eventFromString(message, syntheticException, options);
        esm$1.addExceptionTypeValue(event, message);
        if ('code' in domException) {
    ...
    if (esm$1.isPlainObject(exception) || esm$1.isEvent(exception)) {
        // If it is plain Object or Event, serialize it manually and extract options
        // This will allow us to group events based on top-level keys
        // which is much better than creating new group when any key/value change
        var objectException = exception;
        event = parsers.eventFromPlainObject(objectException, syntheticException, options.rejection);
        esm$1.addExceptionMechanism(event, {
            synthetic: true,
        });
        return event;
    ...
    // So bail out and capture it as a simple message:
    event = eventFromString(exception, syntheticException, options);
    esm$1.addExceptionTypeValue(event, "" + exception, undefined);
    esm$1.addExceptionMechanism(event, {
        synthetic: true,
    });
    return event;
...
unwrapExports(eventbuilder);
var eventbuilder_1 = eventbuilder.eventFromException;
var eventbuilder_2 = eventbuilder.eventFromMessage;
var eventbuilder_3 = eventbuilder.eventFromUnknownInput;
var eventbuilder_4 = eventbuilder.eventFromString;
...
var BaseTransport = /** @class */ (function () {
    function BaseTransport(options) {
        ...
        /** Locks transport after receiving rate limits in a response */
        this._rateLimits = {};
        this._api = new esm$2.API(options.dsn, options._metadata);
        // eslint-disable-next-line deprecation/deprecation
        this.url = this._api.getStoreEndpointWithUrlEncodedAuth();
    }
    /**
    ...
    BaseTransport.prototype._handleResponse = function (_a) {
        var requestType = _a.requestType, response = _a.response, headers = _a.headers, resolve = _a.resolve, reject = _a.reject;
        var status = esm.Status.fromHttpCode(response.status);
        /**
         * "The name is case-insensitive."
         * https://developer.mozilla.org/en-US/docs/Web/API/Headers/get
         */
        var limited = this._handleRateLimit(headers);
        if (limited)
            esm$1.logger.warn("Too many requests, backing off until: " + this._disabledUntil(requestType));
        if (status === esm.Status.Success) {
            resolve({ status: status });
            return;
        }
        reject(response);
    ...
    BaseTransport.prototype._handleRateLimit = function (headers) {
        ...
        if (rlHeader) {
            ...
            this._rateLimits.all = new Date(now + esm$1.parseRetryAfterHeader(now, raHeader));
            return true;
        }
        return false;
    };
...
var FetchTransport = /** @class */ (function (_super) {
     ...
     */
    FetchTransport.prototype.sendEvent = function (event) {
        return this._sendRequest(esm$2.eventToSentryRequest(event, this._api), event);
    };
    /**
     * @inheritDoc
     */
    FetchTransport.prototype.sendSession = function (session) {
        return this._sendRequest(esm$2.sessionToSentryRequest(session, this._api), session);
    };
    /**
     * @param sentryRequest Prepared SentryRequest to be delivered
     * @param originalPayload Original payload used to create SentryRequest
    ...
    FetchTransport.prototype._sendRequest = function (sentryRequest, originalPayload) {
        ...
        return this._buffer.add(new esm$1.SyncPromise(function (resolve, reject) {
            global
                .fetch(sentryRequest.url, options)
                .then(function (response) {
                var headers = {
...
var BrowserBackend = /** @class */ (function (_super) {
     ...
     */
    BrowserBackend.prototype.eventFromException = function (exception, hint) {
        return eventbuilder.eventFromException(this._options, exception, hint);
    };
    /**
     * @inheritDoc
     */
    BrowserBackend.prototype.eventFromMessage = function (message, level, hint) {
        if (level === void 0) { level = esm.Severity.Info; }
        return eventbuilder.eventFromMessage(this._options, message, level, hint);
    };
    /**
     * @inheritDoc
     */
...
})(CordovaPlatformType || (CordovaPlatformType = {}));
...
var processLevel = function (level) {
    if (level === exports.Severity.Critical) {
        return exports.Severity.Fatal;
    }
    if (level === exports.Severity.Log) {
        return exports.Severity.Debug;
    }
    return level;
};
/**
...
var NATIVE = {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                if (this.SUPPORTS_NATIVE_SDK.includes(getPlatform())) {
                    options = __assign({ enableNative: true }, _options);
                    ...
                    beforeSend = options.beforeSend, beforeBreadcrumb = options.beforeBreadcrumb, integrations = options.integrations, defaultIntegrations = options.defaultIntegrations, transport = options.transport, filteredOptions = __rest(options, ["beforeSend", "beforeBreadcrumb", "integrations", "defaultIntegrations", "transport"]);
    ...
    sendEvent: function (event) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                ...
                payload = __assign(__assign({}, event), { message: {
    ...
    setUser: function (user) {
        ...
        if (user) {
            var id = user.id, ip_address = user.ip_address, email = user.email, username = user.username, otherKeys = __rest(user, ["id", "ip_address", "email", "username"]);
    ...
    addBreadcrumb: function (breadcrumb) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        void this._nativeCall('addBreadcrumb', __assign(__assign({}, breadcrumb), { 
            // Process and convert deprecated levels
...
var CordovaTransport = /** @class */ (function () {
    function CordovaTransport(options) {
        ...
        this._buffer = new PromiseBuffer(30);
        if (supportsFetch()) {
            this._fallbackTransport = new transports_2(options);
        }
        else {
            this._fallbackTransport = new transports_3(options);
        }
    }
    /**
     * @inheritDoc
    ...
    CordovaTransport.prototype.close = function (timeout) {
        return Promise.all([this._buffer.drain(timeout), this._fallbackTransport.close()]).then(function (_a) {
            var _b = __read(_a, 2), bufferDrained = _b[0], fallbackClosed = _b[1];
...
var SDK_NAME = 'sentry.javascript.cordova';
var SDK_VERSION$1 = '1.0.0';
...
var DEFAULT_INTEGRATIONS = __spread(defaultIntegrations, [new Cordova()]);
var DEFAULT_OPTIONS = {
    enableNative: true,
...
function init(_options) {
    ...
    var options = __assign(__assign(__assign({}, DEFAULT_OPTIONS), { release: (_a = window === null || window === void 0 ? void 0 : window.SENTRY_RELEASE) === null || _a === void 0 ? void 0 : _a.id }), _options);
    // Initialize a new hub using our scope with native sync
    var cordovaHub = new Hub(undefined, new CordovaScope());
    makeMain(cordovaHub);
